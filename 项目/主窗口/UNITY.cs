    ...
    {       
        ...
        public async Task CallUnityViaTcpAsync(string command, string? data = null, int port = 12345)
        {
            ...
            try
            {
                ...
                if (respLen > 0)
                {
                    ...
                    await stream.ReadAsync(respData);
                ...
                public async Task CallUnityAsync(string command, string? data = null, string pipeName = "ZW_Tool_HighPerf")
        {
            try
            {
                ...
                while (bytesRead < 4)
                {
                    int read = await client.ReadAsync(lenBuf.AsMemory(bytesRead, 4 - bytesRead));
                    if (read == 0) throw new Exception("管道意外关闭");
                    bytesRead += read;
                }
                ...
                {
                    int read = await client.ReadAsync(respBuf.AsMemory(bytesRead, respLen - bytesRead));
                    if (read == 0) throw new Exception("管道意外关闭");
                    bytesRead += read;
                }
